...
dat.raw <-
  readxl::read_excel(
    "../data/Article Coding Sheet 2023-06-20.xlsx",
    sheet = "Symptom Coding",
    range = "C5:J100",
    col_names = TRUE
  ) %>%
  filter(!is.na(`Long Description`))%>%
  select(-2)%>%
  mutate(S = paste0("S",1:nrow(.)))

dat.binary  <- dat.raw %>%
  select(-`Long Description`, -S) %>%
...
```{r}
dat.raw %>%
  select(S, `Long Description`, everything()) %>%
  print(n = 60)
